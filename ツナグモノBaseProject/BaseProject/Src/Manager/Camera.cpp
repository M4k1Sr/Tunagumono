	...
	for (const auto& hitCol : hitColliders_)
	{
		...
		while (sphereCnt < CNT_TRY_COLLISION_CAMERA)
		{
			...
			bool isSphereHit = HitCheck_Sphere_Triangle(
				transform_.pos,                    // 球体(カメラ)の位置
				colliderSphere->GetRadius(),       // 球体の半径
				hitPoly.Position[0],               // 三角形の頂点1
				hitPoly.Position[1],               // 三角形の頂点2
				hitPoly.Position[2]                // 三角形の頂点3
			);

			// 衝突していなければループを抜ける
			if (isSphereHit == false)
			{
				break;
			}

			// 衝突していたら法線方向に押し戻し
			transform_.pos = VAdd(transform_.pos, VScale(hitPoly.Normal, 1.0f));
